...
void RendererVk::initWindow() { glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API); }

void RendererVk::init(GLFWwindow *window, int width, int height,
                      SimParam params) {
   this->params = params;
   texSize = 16;
...
void RendererVk::createRenderPass() {
   ...
   VkFormat formats[] = {swapchain.format};
   VkAttachmentLoadOp loadOps[] = {VK_ATTACHMENT_LOAD_OP_CLEAR};
   ...
   // subpass attachments
   VkAttachmentReference hdr_attach = {
...
void RendererVk::createResources() {
   ...
   computePositionSSBOOffset =
       align(computeUBOOffset + computeUBOSize, minAlignSSBO);
   computePositionSSBOSize = sizeof(glm::vec4) * params.numParticles;
   computeVelocitySSBOOffset =
       align(computePositionSSBOOffset + computePositionSSBOSize, minAlignSSBO);
   ...
   HdrUBO hdrParam;
   hdrParam.flareSize = glm::vec2(texSize / float(2 * swapchain.extent.width),
...
void RendererVk::createBuffers() {
   ...
   createBuffer(bufferSize,
                ...
                VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, localBuffer, localMemory);
...
void RendererVk::createFlare() {
   VkExtent3D extent = {(uint32_t)texSize, (uint32_t)texSize, 1};

   createImage(VK_IMAGE_TYPE_2D, VK_FORMAT_R32_SFLOAT, extent, 1, 1,
               VK_SAMPLE_COUNT_1_BIT, VK_IMAGE_TILING_LINEAR,
               VK_IMAGE_USAGE_TRANSFER_SRC_BIT, VK_IMAGE_LAYOUT_PREINITIALIZED,
               VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                   VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
               flareImageStaging, flareMemoryStaging);

   createImage(VK_IMAGE_TYPE_2D, VK_FORMAT_R32_SFLOAT, extent, 1, 1,
               VK_SAMPLE_COUNT_1_BIT, VK_IMAGE_TILING_OPTIMAL,
               VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
               VK_IMAGE_LAYOUT_UNDEFINED, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
               flareImage, flareMemory);

   // Data copy to staging image
   std::vector<float> pixelData = genFlareTex(texSize);
   void *data;
   vkMapMemory(device, flareMemoryStaging, 0, pixelData.size() * sizeof(float),
               0, &data);
   ...
   transitionImageLayout(
       flareImageStaging, VK_IMAGE_LAYOUT_PREINITIALIZED,
       VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, 0, VK_ACCESS_TRANSFER_READ_BIT,
       VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);

   transitionImageLayout(flareImage, VK_IMAGE_LAYOUT_UNDEFINED,
   ...
   // Final transition
   transitionImageLayout(
       flareImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
void RendererVk::transitionImageLayout(VkImage image, VkImageLayout oldLayout,
   ...
   vkCmdPipelineBarrier(once, srcStageMask, dstStageMask, 0, 0, nullptr, 0,
                        nullptr, 1, &barrier);

   endOnce(once);
}

void RendererVk::copyImage(VkImage srcImage, VkImage dstImage,
                           VkExtent3D extent) {
   VkCommandBuffer once = beginOnce();
   ...
   vkCmdCopyImage(once, srcImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
...
void RendererVk::allocateDescriptorSets() {
   std::vector<VkDescriptorSetLayout> layouts = {hdrSetLayout,
                                                 computeSetLayout};
...
void RendererVk::updateDescriptors() {
   ...
   // HDR parameter UBO
   VkDescriptorBufferInfo hdrParamUBOInfo = {localBuffer, hdrUBOOffset,
                                             hdrUBOSize};
   ...
   // HDR Texture
   VkDescriptorImageInfo imageInfo = {flareSampler, flareView,
   ...
   VkDescriptorBufferInfo computeParamInfo = {localBuffer, computeUBOOffset,
                                              computeUBOSize};
   write = {};
   write.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
   ...
   VkDescriptorBufferInfo computePositionInfo = {
       localBuffer, computePositionSSBOOffset, computePositionSSBOSize};
   write = {};
   write.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
   write.dstSet = descriptorSets.at(1);
   write.dstBinding = 1;
   ...
   VkDescriptorBufferInfo computeVelocityInfo = {
       localBuffer, computeVelocitySSBOOffset, computeVelocitySSBOSize};
   write = {};
   write.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
   write.dstSet = descriptorSets.at(1);
   write.dstBinding = 2;
...
void RendererVk::createGraphicsPipeline() {
   ...
   // Viewport state
   VkViewport viewport = {
       0, 0, (float)swapchain.extent.width, (float)swapchain.extent.height,
       0, 1};
...
void RendererVk::createComputePipelines() {
   std::array<const char *, 2> shaderFilenames = {
       "shaders/vk/interaction_comp.spv", "shaders/vk/integration_comp.spv"};
...
void RendererVk::recordCompute() {
   ...
   vkCmdBindDescriptorSets(computeCmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE,
                           computeLayout, 0, 1, &descriptorSets.at(1), 0,
                           nullptr);
   ...
   for (size_t i = 0; i < params.maxIterationsPerFrame; ++i) {
      ...
      // Interaction
      vkCmdBindPipeline(computeCmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE,
                        computePipelines[0]);
...
void RendererVk::recordDrawing(uint32_t imageIndex) {
   ...
   vkWaitForFences(device, 1, &swapchain.fences[imageIndex], VK_TRUE,
   ...
   // Push constants
   vkCmdPushConstants(cb, hdrLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, 64,
                      glm::value_ptr(viewMat));

   vkCmdPushConstants(cb, hdrLayout, VK_SHADER_STAGE_VERTEX_BIT, 64, 64,
                      glm::value_ptr(projMat));

   // Descriptor sets
   vkCmdBindDescriptorSets(cb, VK_PIPELINE_BIND_POINT_GRAPHICS, hdrLayout, 0, 1,
                           &descriptorSets.at(0), 0, nullptr);
...
void RendererVk::submitDrawing(uint32_t imageIndex) {
   ...
   std::vector<VkSemaphore> waitSemaphores = {swapchain.imageAvailable,
                                              computeFinished};
   std::vector<VkPipelineStageFlags> waitStages = {
       VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
       VK_PIPELINE_STAGE_VERTEX_INPUT_BIT};
   std::vector<VkSemaphore> signalSemaphores = {swapchain.renderFinished,
                                                drawingFinished};
...
void RendererVk::destroy() {
   ...
   for (auto shader : shaders) vkDestroyShaderModule(device, shader, nullptr);
...
void RendererVk::populateParticles(const std::vector<glm::vec4> pos,
   ...
   void *data;
   vkMapMemory(device, hostMemory, computePositionSSBOOffset,
               computePositionSSBOSize, 0, (void **)&data);
   memcpy(data, pos.data(), computePositionSSBOSize);
   vkUnmapMemory(device, hostMemory);

   vkMapMemory(device, hostMemory, computeVelocitySSBOOffset,
               computeVelocitySSBOSize, 0, (void **)&data);
...
void RendererVk::stepSim() { submitCompute(); }
