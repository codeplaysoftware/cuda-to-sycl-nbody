...
/**
 * Returns a score for a physical device - the highest score runs the
 * application A score of -1 is unsuitable, and can't run the application.
 */
int getPhysicalDeviceScore(VkInstance instance,
 ...
 */
QueueFamilies findSuitableQueueFamilies(VkInstance instance,
                                        VkPhysicalDevice physicalDevice);
...
VkSurfaceFormatKHR chooseSurfaceFormat(VkPhysicalDevice physicalDevice,
                                       VkSurfaceKHR surface);
VkPresentModeKHR choosePresentMode(VkPhysicalDevice physicalDevice,
                                   VkSurfaceKHR surface);
VkExtent2D chooseExtent(VkSurfaceCapabilitiesKHR capabilities, int width,
                        int height);
...
void VulkanApp::createSwapchain(int width, int height) {
   ...
   uint32_t queueFamilyIndices[] = {(uint32_t)families.graphics,
   ...
   vkGetSwapchainImagesKHR(device, swapchain.swapchain, &count,
   ...
   for (uint32_t i = 0; i < swapchain.images.size(); ++i) {
      ...
      if (vkCreateImageView(device, &createInfo, nullptr,
                            &swapchain.views.at(i)) != VK_SUCCESS) {
...
void VulkanApp::destroy() {
   ...
   for (auto fence : swapchain.fences) vkDestroyFence(device, fence, nullptr);

   vkFreeCommandBuffers(device, commandPool, swapchain.commandBuffers.size(),
                        swapchain.commandBuffers.data());
...
void VulkanApp::createImage(VkImageType imageType, VkFormat format,
...
bool areDeviceExtensionsSupported(VkPhysicalDevice physicalDevice,
                                  const std::vector<const char *> &extensions) {
   uint32_t propertyCount;
   vkEnumerateDeviceExtensionProperties(physicalDevice, nullptr, &propertyCount,
                                        nullptr);
...
QueueFamilies findSuitableQueueFamilies(VkInstance instance,
   ...
   for (uint32_t i = 0; i < queueFamilyProperties.size(); ++i) {
      if (families.graphics == -1 &&
          queueFamilyProperties.at(i).queueFlags & VK_QUEUE_GRAPHICS_BIT) {
         families.graphics = i;
      }
      if (families.present == -1 && glfwGetPhysicalDevicePresentationSupport(
                                        instance, physicalDevice, i)) {
         families.present = i;
      }
   }
   return families;
...
int getPhysicalDeviceScore(VkInstance instance,
   ...
   vkGetPhysicalDeviceSurfaceSupportKHR(physicalDevice, families.present,
...
VkPhysicalDevice findSuitablePhysicalDevice(
   ...
   for (auto physicalDevice : physicalDevices) {
      int score =
          getPhysicalDeviceScore(instance, extensions, surface, physicalDevice);
...
int32_t VulkanApp::findProperties(VkMemoryPropertyFlags flags) {
